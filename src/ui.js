...
import { getDifficultyColor, isExpertPlus } from "./difficulty-calculator.js";
import {
  loadOsz,
  buildOsz,
  buildOszMulti,
...
export class UI {
  ...
  prewarmFFmpeg() {
    import("./audio-processor.js").then(({ warmupFFmpeg }) => {
      warmupFFmpeg((msg) => console.log("[ffmpeg prewarm]", msg))
        ...
        .catch((err) =>
          console.warn(
            "[ffmpeg] Pre-warm failed (will retry on generate):",
            err.message,
          ),
        );
    });
  ...
  cacheElements() {
    ...
    this.el = {
      ...
      compensateDTCheck: $("compensateDTCheck"),
  ...
  bindEvents() {
    this.el.chooseFileBtn.addEventListener("click", () =>
    ...
    if (isIOS) {
      ...
      this.el.chooseFolderBtn.addEventListener("click", () =>
        this.el.folderInput.click(),
      );
    ...
    this.el.HPSlider.addEventListener("input", (e) =>
      this.editor.setHP(parseFloat(e.target.value)),
    );
    this.el.CSSlider.addEventListener("input", (e) =>
      this.editor.setCS(parseFloat(e.target.value)),
    );
    this.el.ARSlider.addEventListener("input", (e) =>
      this.editor.setAR(parseFloat(e.target.value)),
    );
    this.el.ODSlider.addEventListener("input", (e) =>
      this.editor.setOD(parseFloat(e.target.value)),
    );
    ...
    this.el.BpmLock.addEventListener("click", () =>
      this.editor.toggleBpmLock(),
    );
    ...
    this.el.ScaleARCheck.addEventListener("change", (e) =>
      this.editor.setScaleAR(e.target.checked),
    );
    this.el.ScaleODCheck.addEventListener("change", (e) =>
      this.editor.setScaleOD(e.target.checked),
    );
    ...
    this.el.useOsuFilenameCheck.addEventListener("change", (e) => {
      ...
      this.toast(
        e.target.checked
          ? "Will save .osz as .osu filename"
          : "Will save .osz as Artist - Title",
        "success",
      );
    });

    this.el.compensateDTCheck.addEventListener("change", (e) => {
      this.editor.compensateForDT = e.target.checked;
      this.toast(
        e.target.checked
          ? "DT compensation ON — maps with AR/OD > 10 will be pre-slowed for in-game DT"
          : "DT compensation OFF — AR/OD > 10 written directly",
        "success",
      );
    });
    ...
    this.el.modeLocalBtn.addEventListener("click", () =>
      this.setProcessingMode("local"),
    );
    this.el.modeServerBtn.addEventListener("click", () =>
      this.setProcessingMode("server"),
    );
    ...
    this.el.GenerateMapButton.addEventListener("click", () =>
      this.handleGenerate(),
    );

    this.el.BatchToggleButton.addEventListener("click", () =>
      this.toggleBatchPanel(),
    );
    this.el.BatchCloseButton.addEventListener("click", () =>
      this.toggleBatchPanel(false),
    );
    this.el.BatchGenerateButton.addEventListener("click", () =>
    ...
    this.el.infoBtn.addEventListener("click", () =>
    ...
    document.querySelectorAll(".profile-card").forEach((card) => {
      ...
      card
        .querySelector(".profile-card__save")
        .addEventListener("click", (e) => {
          e.stopPropagation();
          this.editor.saveProfile(idx);
          this.flashProfile(idx);
          this.toast(
            `Saved to ${this.editor.userProfiles[idx].name}`,
            "success",
          );
        });
      card
        .querySelector(".profile-card__load")
      ...
      card
        .querySelector(".profile-card__rename")
        .addEventListener("click", (e) => {
          e.stopPropagation();
          const name = prompt(
            "Rename profile:",
            this.editor.userProfiles[idx].name,
          );
          if (name != null) {
      ...
      card
        .querySelector(".profile-card__delete")
        .addEventListener("click", (e) => {
          e.stopPropagation();
          if (confirm(`Delete "${this.editor.userProfiles[idx].name}"?`)) {
            this.editor.userProfiles[idx] = this.editor.makeEmptyProfile(
              `Profile ${idx + 1}`,
            );
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        if (!this.el.aboutModal.hasAttribute("hidden"))
          this.hideModal("aboutModal");
        if (!this.el.batchPanel.hasAttribute("hidden"))
          this.toggleBatchPanel(false);
      }
    });
  }
  ...
  async handleFile(fileInput) {
    ...
    try {
      ...
      if (osz.beatmaps.length === 0) {
        this.hideProgress();
        this.toast(
          "No .osu files found. Select an .osz/.zip file, or loose .osu + .mp3 files.",
          "error",
        );
        return;
      }
  ...
  renderDiffSelector() {
    ...
    beatmaps.forEach((bm, i) => {
      ...
      chip.className =
        "diff-selector__chip" +
        (i === this.currentDiffIndex ? " is-active" : "");
      chip.dataset.idx = i;
  ...
  switchDifficulty(idx) {
    ...
    if (this.loadedOsz.audioMap) {
      ...
      if (this.loadedOsz.audioMap[audioKey]) {
        ...
        console.log(
          `[audio] Switched to: ${this.loadedOsz.audio.name} for "${bm.version}"`,
        );
      }
    }

    this.el.diffSelectorList
      .querySelectorAll(".diff-selector__chip")
      .forEach((chip, i) => {
  ...
  toggleBatchPanel(force) {
    const show =
      force === undefined ? this.el.batchPanel.hasAttribute("hidden") : force;
  ...
  renderBatchRates() {
    ...
    const allRates = [...new Set([...PRESET_RATES, ...customRates])].sort(
      (a, b) => a - b,
    );
    ...
    for (const rate of allRates) {
      ...
      chip.innerHTML =
        `${rate.toFixed(2)}×` +
        (customRates.includes(rate)
          ? `<span class="batch-rate-chip__remove" data-remove="${rate}" title="Remove">×</span>`
          : "");
  ...
  getCustomRates() {
    try {
      return JSON.parse(
        localStorage.getItem("osutrainer_custom_rates") || "[]",
      );
    } catch {
      return [];
    }
  ...
  saveCustomRates(rates) {
    try {
      localStorage.setItem("osutrainer_custom_rates", JSON.stringify(rates));
    } catch {}
  }
  ...
  renderBatchDiffs() {
    ...
    const allChecked = beatmaps.every((_, i) =>
      this.selectedDiffIndices.has(i),
    );
    allInput.checked = allChecked;
    allInput.addEventListener("change", () => {
      if (allInput.checked) {
        for (let i = 0; i < beatmaps.length; i++)
          this.selectedDiffIndices.add(i);
      } else {
        this.selectedDiffIndices.clear();
      }
      this.renderBatchDiffs();
  ...
  updateBatchSummary() {
    ...
    const audioFiles = rates.filter(
      (r) => Math.abs(r - 1.0) > 0.001 || this.editor.changePitch,
    ).length;
  ...
  async handleBatchGenerate() {
    ...
    const needsAudioCount = rates.filter(
      (r) =>
        audioBlob && (Math.abs(r - 1.0) > 0.001 || this.editor.changePitch),
    ).length;
    ...
    try {
      ...
      const result = await this.editor.generateBatch(
        beatmaps,
        rates,
        audioBlob,
        (p) => {
      ...
      this.showProgress(
        "Packaging…",
        `Building .osz with ${result.osuFiles.length} new files`,
      );
      ...
      this.hideProgress();
      this.toast(
        `Created ${result.osuFiles.length} maps in ${filename}`,
        "success",
      );
  ...
  async handleGenerate() {
    ...
    try {
      ...
      const needsAudio =
        audioBlob &&
        (Math.abs(this.editor.bpmRate - 1.0) > 0.001 ||
          this.editor.changePitch);
      console.log(
        "[generate] needsAudio:",
        needsAudio,
        "bpmRate:",
      ...
      this.showProgress(
        ...
        needsAudio
          ? "Processing audio with ffmpeg.wasm"
          : "Updating difficulty values",
      );

      const result = await this.editor.generateBeatmap(
        audioBlob,
        ({ phase, ratio }) => {
          if (phase === "audio") {
            this.showProgress(
              "Processing audio…",
              `ffmpeg.wasm ${Math.round(ratio * 100)}%`,
            );
      ...
      console.log(
        "[generate] generateBeatmap returned:",
        result ? "OK" : "NULL",
      );

      if (!result) {
        console.warn(
          "[generate] generateBeatmap returned null — state was",
          this.editor.state,
        );
        this.hideProgress();
        return;
  ...
  getDownloadFilename(beatmap) {
    const useOsuName = (() => {
      try {
        return localStorage.getItem("osutrainer_use_osu_filename") === "true";
      } catch {
        return false;
      }
    })();
    ...
    if (useOsuName && beatmap && beatmap.filename) {
      const baseName = beatmap.filename
        .split("/")
        .pop()
        .replace(/\.osu$/i, "");
      return `${baseName}.osz`;
  ...
  renderState() {
    ...
    const inputs = [
      ...
      this.el.ODDisplay,
      this.el.BpmSlider,
      this.el.BpmMultiplierTextBox,
    ];
    const mode = this.editor.getMode();
    const csEnabled =
      ready && (mode === GameMode.osu || mode === GameMode.CatchtheBeat);
    const arEnabled =
      ready && (mode === GameMode.osu || mode === GameMode.CatchtheBeat);
    ...
    const checks = [
      this.el.ScaleARCheck,
      this.el.ScaleODCheck,
      this.el.HRCheck,
      this.el.NoSpinnersCheck,
      this.el.ChangePitchCheck,
      this.el.highQualityMp3Check,
      this.el.compensateDTCheck,
    ];
    checks.forEach((c) => {
      c.disabled = !ready;
    });
  ...
  renderSong() {
    ...
    if (e.starRating > 0) {
      ...
      this._starsAnimCurrent = 0;
      this._animateStars(e.starRating);
      const color = getDifficultyColor(e.starRating);
      this.el.starsBadge.style.color = color;

      this.el.starsBadge.classList.toggle(
        "is-glow",
        isExpertPlus(e.starRating),
      );
    } else {
      this.el.starsBadge.hidden = true;
      this.el.starsBadge.classList.remove("is-glow");
    }
  ...
  renderDifficulty() {
    ...
    set(
      this.el.HPDisplay,
      this.el.HPSlider,
      e.newBeatmap.hpDrainRate,
      e.originalBeatmap.hpDrainRate,
    );
    set(
      this.el.CSDisplay,
      this.el.CSSlider,
      e.newBeatmap.circleSize,
      e.originalBeatmap.circleSize,
    );
    ...
    if (e.starRating > 0) {
      ...
      this.el.starsBadge.hidden = true;
      this.el.starsBadge.classList.remove("is-glow");
      this._starsAnimTarget = 0;
      this._starsAnimCurrent = 0;
    }
  }

  _animateStars(target) {
    this._starsAnimTarget = target;
    if (this._starsAnimCurrent == null) this._starsAnimCurrent = 0;
    if (this._starsAnimRaf != null) return;
    const tick = () => {
      const diff = this._starsAnimTarget - this._starsAnimCurrent;
      if (Math.abs(diff) < 0.005) {
        this._starsAnimCurrent = this._starsAnimTarget;
        this.el.starsValue.textContent = this._starsAnimCurrent.toFixed(2);
        this._starsAnimRaf = null;
        return;
      }
      this._starsAnimCurrent += diff * 0.4;
      this.el.starsValue.textContent = this._starsAnimCurrent.toFixed(2);
      this._starsAnimRaf = requestAnimationFrame(tick);
    };
    this._starsAnimRaf = requestAnimationFrame(tick);
  }
  ...
  renderToggles() {
    ...
    const checks = [
      ...
      [this.el.compensateDTCheck, !!e.compensateForDT],

      [
        this.el.useOsuFilenameCheck,
        (() => {
          try {
            return (
  ...
  async renderMultiThreadedToggle() {
    const { isMultiThreadedEnabled, isMultiThreadingSupported } =
    ...
    console.log(
      `[MT toggle] enabled=${enabled} supported=${supported} SAB=${typeof SharedArrayBuffer !== "undefined"} crossIsolated=${typeof self !== "undefined" ? self.crossOriginIsolated : "?"}`,
    );
    ...
    if (enabled && supported) {
      ...
      status.textContent =
        "⚠ needs COOP/COEP headers — using single-threaded fallback";
    } else {
  ...
  async setMultiThreaded(enabled) {
    const { setMultiThreadedEnabled, isMultiThreadingSupported } =
      await import("./audio-processor.js");
    const supported = isMultiThreadingSupported();
    ...
    this.toast(
      enabled
        ? supported
          ...
          : "Multi-threaded saved (will fall back to ST here)"
  ...
  async testServer() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        this.toast(
          "Server test failed: " + (result.reason || "unknown"),
          "error",
        );
      }
    } catch (e) {
  ...
  renderActions() {
    ...
    this.el.GenerateMapLabel.textContent =
      e.state === EditorState.GENERATING_BEATMAP ? "Working…" : "Create Map";
  ...
  renderProfiles() {
    ...
    document.querySelectorAll(".profile-card").forEach((card, i) => {
      const p = profiles[i];
      if (!p) return;
      card.querySelector(".profile-card__name").textContent =
        p.name || `Profile ${i + 1}`;
  ...
  showProgress(title, subtitle) {
    ...
    if (wasHidden) {
      setTimeout(() => {
        const progressCard =
          this.el.progressOverlay.querySelector(".progress-card");
        if (progressCard) {
